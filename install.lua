...
local RunBot = io.open("JESKA", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/JESKA
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr JESKA.lua
wget "https://raw.githubusercontent.com/JESKA-DEV/JESKA/main/JESKA.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./JESKA.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S JESKA -X kill
screen -S JESKA ./JESKA
done
]])
RunTs:close()
end
